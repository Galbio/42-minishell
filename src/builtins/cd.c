...
/*   Updated: 2025/04/12 14:03:36 by lroussel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	cd_to_home(t_cmd_params *cmd)
{
	...
	if (res)
	{
		translate(2, "command.cd.filedirnotfound", program_arg("cd", new_arg(home_val, NULL)), 1);
		free(home_val);
		return (1);
	}
	res = home_val != NULL;
...
static int	cd_absolute(t_cmd_params *cmd)
{
	...
	if (res < 0)
	{
		translate(2, "command.cd.filedirnotfound", program_arg("cd", new_arg(cmd->argv[1], NULL)), 1);
		return (1);
	}
	change_envp_pwd(cmd, getcwd(NULL, 0));
	return (0);
...
static int	cd_oldpwd(t_cmd_params *cmd)
{
	...
	if (old_pwd)
	{
		...
		if (res < 0)
		{
			translate(2, "command.cd.filedirnotfound", program_arg("cd", new_arg(old_pwd, NULL)), 1);
			return (1);
		}
...
static int	handle_dash(t_cmd_params *cmd)
{
	...
	if ((cmd->argv[1][1] != '-') || (cmd->argv[1][2]))
	{
		v[0] = cmd->argv[1][1];
		v[1] = '\0';
		translate(2, "command.cd.invaliddash", program_arg("cd", new_arg(v, NULL)), 1);
		return (1);
	}
